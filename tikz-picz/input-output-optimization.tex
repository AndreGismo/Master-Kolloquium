...
\begin{tikzpicture}[
		...
		\node[block](block1){\strut Transformatorleistung};		
...
%			below=0.2 of block3](block4){\strut Leitungsimpedanzen};
		...
		\node(gridtopology)[below=0.1 of block3]{\tiny\strut 
		Netztopologie};
		...
		\node[block,
			right=2 of block5, align=center,
		...
		\node[block,
			below=0.5 of GLO](block0){\strut Haushaltslastprofile};
			
		
		\draw[-stealth](block0.north) -- (GLO);
		...
		\begin{pgfonlayer}{background}
			...
			\path(block3.east)+(0.1, -0.9) node (b) {};
